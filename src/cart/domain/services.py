...
class CartService(BaseService):
    ...
    def __init__(self, uow: AbstractUnitOfWork, cart_repo: CartRepositoryI):
        super().__init__(uow)
    ...
    async def add_to_cart(self, dto: AddToCartDTO, session_id: str):
        ...
        await self._cart_repo.add(dto, session_id)

    async def remove_from_cart(self, product_id: int, session_id: str):
        try:
            await self._cart_repo.delete_by_id(product_id, session_id)
    ...
    async def update_product_qty(self, product_id: int, qty: int, session_id: str):
        try:
            if qty == 0:
                return await self._cart_repo.delete_by_id(product_id, session_id)
            return await self._cart_repo.update_qty_by_id(product_id, qty, session_id)
        except NotFoundError:
            raise EntityNotFoundError(self.entity_name, id=product_id)
